...
```text
개발 하며 테스트 및 디버깅, 통계 등의 사용을 위해 로그를 남겨야 해서 사용.
npm install winston-daily-rotate-file : 날짜별 파일 생성
참고 사이트 : https://lovemewithoutall.github.io/it/winston-example/
```
...
즉 NodeJS에서 process.env는 서버의 환경변수를 뜻하며 서버의 환경변수가 미리 설정되어 있다면 타이핑하여 앱을 구동시키지 않아도 된다는 뜻이다. 만약 프로덕션 서버라면 미리 NODE_ENV=production 이라는 환경변수를 만들어 놓고 동작시키면 된다는 뜻이다.
참고사이트 : http://afrobambacar.github.io/2017/03/proccess-env-of-nodejs.html
```

> NODE_ENV 란?
```text
NODE_ENV 값을 이용해서 production 모드와 development 모드를 나누어서 실행한다.
production 모드 일때는 파일 캐싱, 에러 메시지 감추기 등 배포의 적합한 환경 설정을 하고
development 모드 일 때는파일 캐싱 방지, 디버그를 위한 상세한 에러 메시지 보이기 등
개발에 도움을 줄 수 있는 환경으로 설정을 해준다.
참고사이트 : https://steemit.com/kr/@inspiredjw/node-js-nodeenv
```